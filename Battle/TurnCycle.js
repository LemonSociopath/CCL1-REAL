class TurnCycle {
  ...
  async turn() {
    ...
    if (targetDead) {
      await this.onNewEvent({
        type: "textMessage",
        text: `${submission.target.name} has been silenced.`,
      });

      if (submission.target.team === "enemy") {
        const fighters = submission.target.battle.enemy.fighters;
  ...
  async init() {
    await this.onNewEvent({
      type: "textMessage",
      text: `${this.battle.enemy.name} wants to speak their truth.`,
    });
